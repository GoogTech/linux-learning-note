...
3. 修改`crontab`默认编辑器的方法 : 
   ...
   ```shell
   # 在用户 profile 里面添加一行设置 `EDITOR` 的环境变量并使其生效即可，
...
用于对文件中的文本内容进行排序/合并，当然也可对文件进行排序 .
...
```shell
# 查看文件内容
hwte@gl-f1227452macd demo % cat file.txt 
BBB222
...
222BBB
---
...
"""
# 对文件内容进行正向排序（忽略大小写）
hwte@gl-f1227452macd demo % sort -f file.txt 
...
BBB222
# 对文件内容进行逆向排序（忽略大小写）
hwte@gl-f1227452macd demo % sort -fr file.txt 
BBB222
aaa111
222BBB
111aaa
...
hwte@gl-f1227452macd demo % sort -frn file.txt
222BBB
111aaa
aaa111
BBB222
